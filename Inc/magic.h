...
#ifndef __AI_MAGIC_H__
#define __AI_MAGIC_H__
#pragma once

#include "ai_platform.h"

#define AI_MAGIC_MODEL_NAME          "magic"

#define AI_MAGIC_IN_NUM       (1)
#define AI_MAGIC_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 128, 3, 1, 1, NULL), \
}
#define AI_MAGIC_IN_SIZE { \
  (128 * 3 * 1), \
}
#define AI_MAGIC_IN_1_SIZE  (128 * 3 * 1)
#define AI_MAGIC_IN_1_SIZE_BYTES  ((128 * 3 * 1) * 4)




#define AI_MAGIC_OUT_NUM      (1)
#define AI_MAGIC_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 4, 1, NULL), \
}
#define AI_MAGIC_OUT_SIZE { \
  (1 * 1 * 4), \
}
#define AI_MAGIC_OUT_1_SIZE  (1 * 1 * 4)
#define AI_MAGIC_OUT_1_SIZE_BYTES  ((1 * 1 * 4) * 4)
 ...
 * @ref ai_magic_create. Both the activations memory buffer 
 ...
 * header generated by the code generation tool. See AI_MAGIC_*
 * defines into file @ref magic.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_magic_forward(
  ai_handle network, const ai_buffer* input);


AI_API_DECLARE_END

#endif /*__AI_MAGIC_H__*/
